...
-- gnaf_202602.address_principal_admin_boundaries: DERIVED via spatial join (E1.10)
-- Data is no longer pre-baked. The spatial join fallback in address_full_prep.sql
-- populates this table from admin_bdys boundary polygons loaded by seed-admin-bdys.sql
-- and prepared by prep-admin-bdys.sql.
...
SELECT setval('gnaf_202602.address_principal_admin_boundaries_gid_seq', GREATEST(1, (SELECT COALESCE(MAX(gid), 0) FROM gnaf_202602.address_principal_admin_boundaries)));
