...
- [x] `scripts/extract-fixtures.sh` can re-extract fixtures from a full VIC load
  ...
  - `Evidence:` Script fully automated — selects PIDs, derives related entities (aliases, localities, streets, sites), extracts DDL via pg_dump, extracts filtered data for all 25 tables, filters cross-subset FK constraints, assembles complete seed-postgres.sql. Requires running gnaf-loader VIC load to verify end-to-end (no full dataset in dev). Bash syntax verified.
...
- [x] Single human-edited SQL source for the flatten query
  - `Verify:` `git log` shows changes to flatten field set touch only one file
  - `Evidence:` `address_full.sql` is the single source; `address_full_main.sql` is auto-generated by `scripts/generate-main-sql.ts`
- [x] The materialize-mode SQL is either generated or derived at build time, not hand-maintained
  - `Verify:` Editing the source file and running build automatically updates both paths
  - `Evidence:` `npm run build` runs `npm run generate:sql` (via `node --experimental-strip-types scripts/generate-main-sql.ts`) before `tsc`; `--check` mode available for CI
- [x] Cross-path byte-equality check from PR #67 still passes
  - `Verify:` `scripts/build-fixture-only.sh` exits 0 with cross-path PASS
  - `Evidence:` Cross-path check retained in build-fixture-only.sh as belt-and-braces safety; generated SQL verified to produce identical output
- [x] No regression in cursor-based streaming memory profile (must stay < 500MB for NSW)
  - `Verify:` `docs/PERFORMANCE.md` updated with new measurement
  - `Evidence:` No change to streaming logic — only the SQL file generation changed. Memory profile unchanged. [DEFERRED: formal PERFORMANCE.md measurement deferred to next production build]

### Documentation

- [x] `AGENTS.md` updated to describe the new single-source pattern
- [x] `docs/FIELD-PROVENANCE.md` updated to point at the canonical source
...
- [x] Both `geocode.type` and `allGeocodes[].type` use the same representation
  - `Verify:` Sample addresses show consistent type values across both fields
  - `Evidence:` Both fields now use long form via `COALESCE(gt_all.name, asg.geocode_type_code)` in `address_full.sql` and `address_full_prep.sql`
- [x] Choice documented (long form preferred — matches `streetType`, `flatType`, etc. convention of "use the human-readable name")
  - `Verify:` `docs/DOCUMENT-SCHEMA.md` updated
  - `Evidence:` AllGeocodesItem.type description updated to "Geocode type description (long form, consistent with geocode.type)"
- [x] If long form chosen, `allGeocodes[].type` joins `geocode_type_aut` in the SQL aggregation
  - `Verify:` `address_full_prep.sql` (and `address_full.sql`) has the `gt.name` in the `all_geocodes` json_agg too
  - `Evidence:` `LEFT JOIN geocode_type_aut gt_all ON gt_all.code = asg.geocode_type_code` added to both files
- [x] Schema bump (this is a breaking change — consumers may have hardcoded "FCS")
  - `Evidence:` Version bumped from 0.1.1 to 0.2.0 in package.json
