...
- **Quarterly build "permanent fix" — two unrelated bugs causing all 9 build jobs to fail:**
  ...
  **Permanent fixes (no band-aids):**
  1. **Replaced narrow `grep -qE 'address_alias_admin_boundaries.*(ward_pid|se_upper_pid)'` retry hack in `docker-entrypoint.sh` with a broad Part-5 detection** (`scripts/detect-load-failure.sh`). New detection condition: if gnaf-loader exits non-zero AND the log contains `Part 5 of 6 : Start boundary tagging`, retry with `--no-boundary-tag` and let flat-white's spatial-join fallback in `address_full_prep.sql` populate boundaries. Catches any future Part 5 error, not just specific column names. When upstream `minus34/gnaf-loader` ships a fix (E1.20), the first attempt succeeds and the fallback never fires. **Tested** by `test/integration/load-detection/test.sh` against 8 sample log fixtures (success, all 5 known column failures, 1 hypothetical future failure, 2 negative cases).
