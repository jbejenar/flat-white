...
> **Patch release** — supersedes the deleted v2026.04 release. The underlying G-NAF data is unchanged (still G-NAF 2026.02). v2026.04 was published 2026-04-05 with all-null boundary fields (lga, ward, stateElectorate, commonwealthElectorate) due to upstream gnaf-loader Part 5 failures that flat-white's fallback path was too slow to handle in time. v2026.02.1 ships the same address data with correct, populated boundary fields via the new bulk-join spatial fallback (E1.21 / PR #106). v2026.04 was deleted from GitHub Releases on 2026-04-09; its git tag survives for history.
...
- **E1.21 quarterly impact:** Quarterly build run [24163471133](https://github.com/jbejenar/flat-white/actions/runs/24163471133) is the **first quarterly run with all 9 states green**. NSW dropped from 1h56m (failed in flatten with `tmp_address_geocodes does not exist` after the slow spatial join) to 29m20s (clean pass). SA dropped from 55m15s (failed) to 12m54s. TAS dropped from exit 3 (failed) to 11m30s. The OLD LATERAL+LIMIT code was crashing 3 of 9 states (NSW/SA/TAS) due to a separate latent flatten-session bug (E1.24, masked by E1.21).
...
- This is a patch release. The underlying G-NAF data is unchanged from v2026.02 (which itself was unchanged in v2026.04 due to E1.17 hardcoded download URL). Asset filenames are versioned as `flat-white-2026.02.1-{state}.ndjson.gz` so consumers can detect that previous downloads are stale.
- **v2026.04 was deleted on 2026-04-09.** It shipped with all-null boundary fields and was superseded by this release. Git tag `v2026.04` survives for history; the GitHub release page is gone (404).
...
- Discovered side effects (filed for follow-up): WA cursor stream is 40-75× slower when restored from cache vs fresh build (E1.26); release workflow CHANGELOG push fails on branch protection (E1.27 — this very entry was manually added because of that bug); catalogue workflow has not been triggered for any release because of GitHub Actions GITHUB_TOKEN recursion prevention (E1.28).

## [Unreleased — superseded entries below preserved for context]
